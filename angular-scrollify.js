      ...
      return {
        ...
        compile: function (_element, _attr, linker) {
          return function link($scope, $element, $attrs) {
            ...
            if (!match) {
              $log.error(
                'Expected hjScrollify in form of "_item_ in _array_" but got "' +
                  expression +
                  '".'
              );
            }
            ...
            var $templatePane = angular.element(
              scrollifyEl.querySelector('.scrollify__pane')
            );
            ...
            var moveSlider = function (transitionDuration) {
              ...
              $timeout(function () {
                slider.style[prefixedTransitionDuration] =
                  transitionDuration + 'ms';

                slider.style[prefixedTransform] =
                  'translate(0px, ' +
                  (currentPane / panes.length) * -100 +
                  '%)';
            ...
            var debounceScrollToCurrent = debounce(
              options.scrollSpeed,
              function () {
                preventScroll = false;
              }
              ...
              speed =
                speed !== undefined
                  ...
                  : Math.round(
                      Math.max(
                        1,
                        calcRoot(distance, options.scrollSpeedModifier)
                      )
                    ) * options.scrollSpeed;
              ...
              if (!isTouch) {
                angular
                  ...
                  .css(
                    ...
                    Math.max(
                      200,
                      list.length * options.scrollBarModifier * 100
                    ) + '%'
                  );
              }
            };
            ...
            var wheelHandler = function (event) {
              ...
              var touchPad = isTouchpad(
                event.wheelDeltaY || event.wheelDelta || event.detail || 0
              );
            ...
            var debounceScroll = debounce(defaults.scrollDebounce, function () {
              ...
              var speed =
                Math.round(
                  Math.max(1, calcRoot(distance, options.scrollSpeedModifier))
                ) * options.scrollSpeed;
